...
def camera_1_callback(image):
    file_name = f"sensors/camera_1/vehicle-id-{vehicle.id}-frame-{image.frame}.png"
...
def camera_2_callback(image):
    file_name = f"sensors/camera_2/vehicle-id-{vehicle.id}-frame-{image.frame}.png"
    image.save_to_disk(file_name)
    client_apm.begin_transaction(transaction_type="track-do-thing")
    client_apm.capture_message(message="Camera file saved to storage:\n"+file_name+'\n')
    client_apm.end_transaction("carla_transaction", "success")
...
camera_1.listen(lambda image: camera_1_callback(image))
camera_2.listen(lambda image: camera_2_callback(image))
