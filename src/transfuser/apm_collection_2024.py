...
def camera_callback(image, number):
    file_name = f"sensors/camera_{number}/vehicle-id-{vehicle.id}-frame-{image.frame}.png"
    image.save_to_disk(file_name)
    client_apm.begin_transaction(transaction_type="track-do-thing")
    client_apm.capture_message(message="Camera file saved to storage:\n"+file_name+'\n')
...
def lidar_callback(data):
    ...
    client_apm.begin_transaction(transaction_type="track-do-thing")
    client_apm.capture_message(message="point cloud file saved to storage:\n"+file_name+'\n')
    client_apm.end_transaction("carla_transaction", "success")
...
def gnss_callback(data):
    ...
    client_apm.begin_transaction(transaction_type="track-do-thing")
    client_apm.capture_message(message="GNSS data file saved to storage:\n"+file_name+'\n')
    client_apm.end_transaction("carla_transaction", "success")
...
def imu_callback(data):
    ...
    client_apm.begin_transaction(transaction_type="track-do-thing")
    client_apm.capture_message(message="IMU and speed data file saved to storage:\n"+file_name+'\n')
    client_apm.end_transaction("carla_transaction", "success")
...
camera_1.listen(lambda image: camera_callback(image, "1"))
camera_2.listen(lambda image: camera_callback(image, "2"))
camera_3.listen(lambda image: camera_callback(image, "3"))
